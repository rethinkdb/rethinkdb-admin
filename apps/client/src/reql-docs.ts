...
const reql_docs = {
  ...
  'api/javascript/append/': {
    ...
    example:
      "<p><strong>Example:</strong> Retrieve Iron Man's equipment list with the addition of some new boots.</p>\n<pre><code>r.table('marvel').get('IronMan')('equipment').append('newBoots').run(conn, callback)\n</code></pre>",
    io: [['array', 'array']],
    name: 'append',
    url: 'append',
  },
  ...
  'api/javascript/bit_not/': {
    ...
    description:
      '<p>Compute the arithmetic inverse (not) of an expression.</p>',
    example:
  ...
  'api/javascript/bit_sar/': {
    ...
    description:
      '<p>Compute the right arithmetic shift of one or more values.</p>',
    example:
  ...
  'api/javascript/bracket/': {
    ...
    example:
      "<p><strong>Example:</strong> What was Iron Man's first appearance in a comic?</p>\n<pre><code>r.table('marvel').get('IronMan')('firstAppearance').run(conn, callback)\n</code></pre>",
    io: [
      ['sequence', 'sequence'],
  ...
  'api/javascript/ceil/': {
    ...
    description:
      "<p>Rounds the given value up, returning the smallest integer value greater than or equal to the given value (the value's ceiling).</p>",
    example:
      '<p><strong>Example:</strong> Return the ceiling of 12.345.</p>\n<pre><code>r.ceil(12.345).run(conn, callback);\n// Result passed to callback\n13.0\n</code></pre>\n<p>The <code>ceil</code> command can also be chained after an expression.</p>',
    io: [['number', 'number']],
    name: 'ceil',
    url: 'ceil',
  },
  ...
  'api/javascript/circle/': {
    ...
    example:
      "<p><strong>Example:</strong> Define a circle.</p>\n<pre><code>r.table('geo').insert({\n    id: 300,\n    name: 'Hayes Valley',\n    neighborhood: r.circle([-122.423246,37.779388], 1000)\n}).run(conn, callback);\n</code></pre>",
    io: [['r', 'geometry']],
    name: 'circle',
    url: 'circle',
  },
  ...
  'api/javascript/config/': {
    ...
    example:
      "<p><strong>Example:</strong> Get the configuration for the <code>users</code> table.</p>\n<pre><code>&gt; r.table('users').config().run(conn, callback);\n</code></pre>",
    io: [
  ...
  'api/javascript/count/': {
    ...
    example:
      "<p><strong>Example:</strong> Count the number of users.</p>\n<pre><code>r.table('users').count().run(conn, callback);\n</code></pre>",
    io: [
      ['sequence', 'number'],
      ['binary', 'number'],
    ],
  ...
  'api/javascript/date/': {
    ...
    description:
      '<p>Return a new time object only based on the day, month and year (ie. the same day at 00:00).</p>',
    example:
      '<p><strong>Example:</strong> Retrieve all the users whose birthday is today.</p>\n<pre><code>r.table("users").filter(function(user) {\n    return user("birthdate").date().eq(r.now().date())\n}).run(conn, callback)\n</code></pre>',
    io: [['time', 'time']],
    name: 'date',
    url: 'date',
  },
  ...
  'api/javascript/day_of_week/': {
    ...
    description:
      '<p>Return the day of week of a time object as a number between 1 and 7 (following ISO 8601 standard). For your convenience, the terms r.monday, r.tuesday etc. are defined and map to the appropriate integer.</p>',
    example:
      "<p><strong>Example:</strong> Return today's day of week.</p>\n<pre><code>r.now().dayOfWeek().run(conn, callback)\n</code></pre>",
    io: [['time', 'number']],
    name: 'dayOfWeek',
    url: 'day_of_week',
  },
  'api/javascript/day_of_year/': {
    ...
    description:
      '<p>Return the day of the year of a time object as a number between 1 and 366 (following ISO 8601 standard).</p>',
    example:
      '<p><strong>Example:</strong> Retrieve all the users who were born the first day of a year.</p>\n<pre><code>r.table("users").filter(\n    r.row("birthdate").dayOfYear().eq(1)\n)\n</code></pre>',
    io: [['time', 'number']],
    name: 'dayOfYear',
    url: 'day_of_year',
  },
  ...
  'api/javascript/delete_at/': {
    ...
    description:
      '<p>Remove one or more elements from an array at a given index. Returns the modified array. (Note: <code>deleteAt</code> operates on arrays, not documents; to delete documents, see the <a href="/api/javascript/delete">delete</a> command.)</p>',
    example:
      "<p><strong>Example:</strong> Delete the second element of an array.</p>\n<pre><code>&gt; r(['a','b','c','d','e','f']).deleteAt(1).run(conn, callback)\n// result passed to callback\n['a', 'c', 'd', 'e', 'f']\n</code></pre>",
    io: [['array', 'array']],
    name: 'deleteAt',
    url: 'delete_at',
  },
  'api/javascript/difference/': {
    ...
    example:
      "<p><strong>Example:</strong> Retrieve Iron Man's equipment list without boots.</p>\n<pre><code>r.table('marvel').get('IronMan')('equipment')\n  .difference(['Boots'])\n  .run(conn, callback)\n</code></pre>",
    io: [['array', 'array']],
    name: 'difference',
    url: 'difference',
  },
  ...
  'api/javascript/div/': {
    ...
    example:
      "<p><strong>Example:</strong> It's as easy as 2 / 2 = 1.</p>\n<pre><code>r.expr(2).div(2).run(conn, callback)\n</code></pre>",
    io: [['number', 'number']],
    name: 'div',
    url: 'div',
  },
  ...
  'api/javascript/each/': {
    ...
    example:
      "<p><strong>Example:</strong> Let's process all the elements!</p>\n<pre><code>cursor.each(function(err, row) {\n    if (err) throw err;\n    processRow(row);\n});\n</code></pre>",
    io: [['cursor', 'undefined']],
    name: 'each',
    url: 'each',
  },
  'api/javascript/each_async/': {
    ...
    description:
      '<p>Lazily iterate over a cursor, array or feed one element at a time. <code>eachAsync</code> always returns a promise that will be resolved once all rows are returned.</p>',
    example:
      "<p><strong>Example:</strong> Process all the elements in a stream, using <code>then</code> and <code>catch</code> for handling the end of the stream and any errors. Note that iteration may be stopped in the first callback (<code>rowProcess</code>) by returning any non-Promise value.</p>\n<pre><code>cursor.eachAsync(function (row) {\n    var ok = processRowData(row);\n    if (!ok) {\n        return 'Bad row: ' + row;\n    }\n}).then(function () {\n    console.log('done processing');\n}).catch(function (error) {\n    console.log('Error:', error.message);\n});\n</code></pre>",
    io: [['cursor', 'undefined']],
    name: 'eachAsync',
    url: 'each_async',
  },
  ...
  'api/javascript/epoch_time/': {
    ...
    description:
      '<p>Create a time object based on seconds since epoch. The first argument is a double and\nwill be rounded to three decimal places (millisecond-precision).</p>',
    example:
      '<p><strong>Example:</strong> Update the birthdate of the user "John" to November 3rd, 1986.</p>\n<pre><code>r.table("user").get("John").update({birthdate: r.epochTime(531360000)}).run(conn, callback)\n</code></pre>',
    io: [['r', 'time']],
    name: 'epochTime',
    url: 'epoch_time',
  },
  'api/javascript/eq/': {
    ...
    example:
      "<p><strong>Example:</strong> See if a user's <code>role</code> field is set to <code>administrator</code>.</p>\n<pre><code>r.table('users').get(1)('role').eq('administrator').run(conn, callback);\n</code></pre>",
    io: [['value', 'bool']],
    name: 'eq',
    url: 'eq',
  },
  ...
  'api/javascript/error/': {
    ...
    description:
      '<p>Throw a runtime error. If called with no arguments inside the second argument to <code>default</code>, re-throw the current error.</p>',
    example:
      "<p><strong>Example:</strong> Iron Man can't possibly have lost a battle:</p>\n<pre><code>r.table('marvel').get('IronMan').do(function(ironman) {\n    return r.branch(ironman('victories').lt(ironman('battles')),\n        r.error('impossible code path'),\n        ironman)\n}).run(conn, callback)\n</code></pre>",
    io: [['r', 'error']],
    name: 'error',
    url: 'error',
  },
  'api/javascript/event_emitter/': {
    ...
    example:
      "<p><strong>Example:</strong> Monitor the connection state with events.</p>\n<pre><code>r.connect({}, function(err, conn) {\n    if (err) throw err;\n\n    conn.addListener('error', function(e) {\n        processNetworkError(e);\n    });\n\n    conn.addListener('close', function() {\n        cleanup();\n    });\n\n    runQueries(conn);\n});\n</code></pre>",
  ...
  'api/javascript/expr/': {
    ...
    example:
      "<p><strong>Example:</strong> Objects wrapped with <code>expr</code> can then be manipulated by ReQL API functions.</p>\n<pre><code>r.expr({a:'b'}).merge({b:[1,2,3]}).run(conn, callback)\n</code></pre>",
    io: [['r', 'value']],
    name: 'expr',
    url: 'expr',
  },
  'api/javascript/fill/': {
    ...
    description:
      '<p>Convert a Line object into a Polygon object. If the last point does not specify the same coordinates as the first point, <code>polygon</code> will close the polygon by connecting them.</p>',
    example:
      "<p><strong>Example:</strong> Create a line object and then convert it to a polygon.</p>\n<pre><code>r.table('geo').insert({\n    id: 201,\n    rectangle: r.line(\n        [-122.423246,37.779388],\n        [-122.423246,37.329898],\n        [-121.886420,37.329898],\n        [-121.886420,37.779388]\n    )\n}).run(conn, callback);\n\nr.table('geo').get(201).update({\n    rectangle: r.row('rectangle').fill()\n}, {nonAtomic: true}).run(conn, callback);\n</code></pre>",
    io: [['line', 'polygon']],
    name: 'fill',
    url: 'fill',
  },
  ...
  'api/javascript/floor/': {
    ...
    description:
      "<p>Rounds the given value down, returning the largest integer value less than or equal to the given value (the value's floor).</p>",
    example:
      '<p><strong>Example:</strong> Return the floor of 12.345.</p>\n<pre><code>r.floor(12.345).run(conn, callback);\n// Result passed to callback\n12.0\n</code></pre>\n<p>The <code>floor</code> command can also be chained after an expression.</p>',
    io: [['number', 'number']],
    name: 'floor',
    url: 'floor',
  },
  ...
  'api/javascript/for_each/': {
    ...
    example:
      "<p><strong>Example:</strong> Now that our heroes have defeated their villains, we can safely remove them from the villain table.</p>\n<pre><code>r.table('marvel').forEach(function(hero) {\n    return r.table('villains').get(hero('villainDefeated')).delete()\n}).run(conn, callback)\n</code></pre>",
    io: [['sequence', 'object']],
    name: 'forEach',
    url: 'for_each',
  },
  'api/javascript/ge/': {
    ...
    description:
      '<p>Compare values, testing if the left-hand value is greater than or equal to the right-hand.</p>',
    example:
      "<p><strong>Example:</strong> Test if a player has scored 10 points or more.</p>\n<pre><code>r.table('players').get(1)('score').ge(10).run(conn, callback);\n</code></pre>",
    io: [['value', 'bool']],
    name: 'ge',
    url: 'ge',
  },
  'api/javascript/geojson/': {
    ...
    description:
      '<p>Convert a <a href="http://geojson.org">GeoJSON</a> object to a ReQL geometry object.</p>',
    example:
      "<p><strong>Example:</strong> Convert a GeoJSON object to a ReQL geometry object.</p>\n<pre><code>var geoJson = {\n    'type': 'Point',\n    'coordinates': [ -122.423246, 37.779388 ]\n};\nr.table('geo').insert({\n    id: 'sfo',\n    name: 'San Francisco',\n    location: r.geojson(geoJson)\n}).run(conn, callback);\n</code></pre>",
    io: [['r', 'geometry']],
    name: 'geojson',
    url: 'geojson',
  },
  ...
  'api/javascript/grant/': {
    ...
    example:
      '<p><strong>Example:</strong> Grant the <code>chatapp</code> user account read and write permissions on the <code>users</code> database.</p>\n<pre><code>r.db(\'users\').grant(\'chatapp\', {read: true, write: true}).run(conn, callback);\n\n// Result passed to callback\n{\n    "granted": 1,\n    "permissions_changes": [\n        {\n            "new_val": { "read": true, "write": true },\n            "old_val": { null }\n        }\n    ]\n</code></pre>',
    io: [
      ['r', 'object'],
      ['table', 'object'],
      ['db', 'object'],
  ...
  'api/javascript/gt/': {
    ...
    description:
      '<p>Compare values, testing if the left-hand value is greater than the right-hand.</p>',
    example:
      "<p><strong>Example:</strong> Test if a player has scored more than 10 points.</p>\n<pre><code>r.table('players').get(1)('score').gt(10).run(conn, callback);\n</code></pre>",
    io: [['value', 'bool']],
    name: 'gt',
    url: 'gt',
  },
  'api/javascript/has_fields/': {
    ...
    example:
      "<p><strong>Example:</strong> Return the players who have won games.</p>\n<pre><code>r.table('players').hasFields('games_won').run(conn, callback)\n</code></pre>",
    io: [
      ['sequence', 'stream'],
      ['array', 'array'],
  ...
  'api/javascript/hours/': {
    ...
    example:
      '<p><strong>Example:</strong> Return all the posts submitted after midnight and before 4am.</p>\n<pre><code>r.table("posts").filter(function(post) {\n    return post("date").hours().lt(4)\n})\n</code></pre>',
    io: [['time', 'number']],
    name: 'hours',
    url: 'hours',
  },
  'api/javascript/http/': {
    ...
    example:
      "<p><strong>Example:</strong> Perform an HTTP <code>GET</code> and store the result in a table.</p>\n<pre><code>r.table('posts').insert(r.http('http://httpbin.org/get')).run(conn, callback)\n</code></pre>",
    io: [
      ['r', 'value'],
      ['r', 'stream'],
    ],
  ...
  'api/javascript/in_timezone/': {
    ...
    description:
      '<p>Return a new time object with a different timezone. While the time stays the same, the results returned by methods such as hours() will change since they take the timezone into account. The timezone argument has to be of the ISO 8601 format.</p>',
    example:
      "<p><strong>Example:</strong> Hour of the day in San Francisco (UTC/GMT -8, without daylight saving time).</p>\n<pre><code>r.now().inTimezone('-08:00').hours().run(conn, callback)\n</code></pre>",
    io: [['time', 'time']],
    name: 'inTimezone',
    url: 'in_timezone',
  },
  ...
  'api/javascript/info/': {
    ...
    example:
      "<p><strong>Example:</strong> Get information about a table such as primary key, or cache size.</p>\n<pre><code>r.table('marvel').info().run(conn, callback)\n</code></pre>",
    io: [['any', 'object']],
    name: 'info',
    url: 'info',
  },
  ...
  'api/javascript/intersects/': {
    ...
    example:
      '<p><strong>Example:</strong> Is <code>point2</code> within a 2000-meter circle around <code>point1</code>?</p>\n<pre><code>var point1 = r.point(-117.220406,32.719464);\nvar point2 = r.point(-117.206201,32.725186);\nr.circle(point1, 2000).intersects(point2).run(conn, callback);\n// result returned to callback\ntrue\n</code></pre>',
    io: [
      ['geometry', 'bool'],
      ['sequence', 'sequence'],
    ],
  ...
  'api/javascript/iso8601/': {
    ...
    description:
      '<p>Create a time object based on an ISO 8601 date-time string (e.g. \'2013-01-01T01:01:01+00:00\'). RethinkDB supports all valid ISO 8601 formats except for week dates. Read more about the ISO 8601 format at <a href="http://en.wikipedia.org/wiki/ISO_8601">Wikipedia</a>.</p>',
    example:
      '<p><strong>Example:</strong> Update the time of John\'s birth.</p>\n<pre><code>r.table("user").get("John").update({birth: r.ISO8601(\'1986-11-03T08:30:00-07:00\')}).run(conn, callback)\n</code></pre>',
    io: [['r', 'time']],
    name: 'ISO8601',
    url: 'iso8601',
  },
  'api/javascript/js/': {
    ...
    example:
      "<p><strong>Example:</strong> Concatenate two strings using JavaScript.</p>\n<pre><code>r.js(\"'str1' + 'str2'\").run(conn, callback)\n</code></pre>",
    io: [['r', 'value']],
    name: 'js',
    url: 'js',
  },
  'api/javascript/json/': {
    ...
    example:
      '<p><strong>Example:</strong> Send an array to the server.</p>\n<pre><code>r.json("[1,2,3]").run(conn, callback)\n</code></pre>',
    io: [['r', 'value']],
    name: 'json',
    url: 'json',
  },
  'api/javascript/keys/': {
    ...
    description:
      '<p>Return an array containing all of an object\'s keys. Note that the keys will be sorted as described in <a href="/docs/data-types/#sorting-order">ReQL data types</a> (for strings, lexicographically).</p>',
    example:
      '<p><strong>Example:</strong> Get all the keys from a table row.</p>\n<pre><code>// row: { id: 1, mail: "fred@example.com", name: "fred" }\n\nr.table(\'users\').get(1).keys().run(conn, callback);\n// Result passed to callback\n[ "id", "mail", "name" ]\n</code></pre>',
    io: [
  ...
  'api/javascript/le/': {
    ...
    description:
      '<p>Compare values, testing if the left-hand value is less than or equal to the right-hand.</p>',
    example:
      "<p><strong>Example:</strong> Test if a player has scored 10 points or less.</p>\n<pre><code>r.table('players').get(1)('score').le(10).run(conn, callback);\n</code></pre>",
    io: [['value', 'bool']],
    name: 'le',
    url: 'le',
  },
  ...
  'api/javascript/line/': {
    ...
    example:
      "<p><strong>Example:</strong> Define a line.</p>\n<pre><code>r.table('geo').insert({\n    id: 101,\n    route: r.line([-122.423246,37.779388], [-121.886420,37.329898])\n}).run(conn, callback);\n</code></pre>",
    io: [['r', 'line']],
    name: 'line',
    url: 'line',
  },
  'api/javascript/literal/': {
    ...
    description:
      '<p>Replace an object in a field instead of merging it with an existing object in a <code>merge</code> or <code>update</code> operation. Using <code>literal</code> with no arguments in a <code>merge</code> or <code>update</code> operation will remove the corresponding field.</p>',
    example:
      '<p><strong>Example:</strong> Replace one nested document with another rather than merging the fields.</p>\n<pre><code>r.table(\'users\').get(1).update({ data: r.literal({ age: 19, job: \'Engineer\' }) }).run(conn, callback)\n\n// Result passed to callback\n{\n    "id": 1,\n    "name": "Alice",\n    "data": {\n        "age": 19,\n        "job": "Engineer"\n    }\n}\n</code></pre>',
    io: [['r', 'special']],
    name: 'literal',
    url: 'literal',
  },
  'api/javascript/lt/': {
    ...
    description:
      '<p>Compare values, testing if the left-hand value is less than the right-hand.</p>',
    example:
      "<p><strong>Example:</strong> Test if a player has scored less than 10 points.</p>\n<pre><code>r.table('players').get(1)('score').lt(10).run(conn, callback);\n</code></pre>",
    io: [['value', 'bool']],
    name: 'lt',
    url: 'lt',
  },
  ...
  'api/javascript/minutes/': {
    ...
    example:
      '<p><strong>Example:</strong> Return all the posts submitted during the first 10 minutes of every hour.</p>\n<pre><code>r.table("posts").filter(function(post) {\n    return post("date").minutes().lt(10)\n})\n</code></pre>',
    io: [['time', 'number']],
    name: 'minutes',
    url: 'minutes',
  },
  ...
  'api/javascript/month/': {
    ...
    description:
      '<p>Return the month of a time object as a number between 1 and 12. For your convenience, the terms r.january, r.february etc. are defined and map to the appropriate integer.</p>',
    example:
      '<p><strong>Example:</strong> Retrieve all the users who were born in November.</p>\n<pre><code>r.table("users").filter(\n    r.row("birthdate").month().eq(11)\n)\n</code></pre>',
    io: [['time', 'number']],
    name: 'month',
    url: 'month',
  },
  ...
  'api/javascript/ne/': {
    ...
    example:
      "<p><strong>Example:</strong> See if a user's <code>role</code> field is not set to <code>administrator</code>.</p>\n<pre><code>r.table('users').get(1)('role').ne('administrator').run(conn, callback);\n</code></pre>",
    io: [['value', 'bool']],
    name: 'ne',
    url: 'ne',
  },
  ...
  'api/javascript/noreply_wait/': {
    ...
    description:
      '<p><code>noreplyWait</code> ensures that previous queries with the <code>noreply</code> flag have been processed by the server. Note that this guarantee only applies to queries run on the given connection.</p>\n<p>If no callback is provided, a promise will be returned.</p>',
    example:
  ...
  'api/javascript/now/': {
    ...
    description:
      '<p>Return a time object representing the current time in UTC. The command now() is computed once when the server receives the query, so multiple instances of r.now() will always return the same time inside a query.</p>',
    example:
      '<p><strong>Example:</strong> Add a new user with the time at which he subscribed.</p>\n<pre><code>r.table("users").insert({\n    name: "John",\n    subscription_date: r.now()\n}).run(conn, callback)\n</code></pre>',
    io: [['r', 'time']],
    name: 'now',
    url: 'now',
  },
  ...
  'api/javascript/point/': {
    ...
    description:
      '<p>Construct a geometry object of type Point. The point is specified by two floating point numbers, the longitude (&minus;180 to 180) and latitude (&minus;90 to 90) of the point on a perfect sphere. See <a href="/docs/geo-support/">Geospatial support</a> for more information on ReQL\'s coordinate system.</p>',
    example:
      "<p><strong>Example:</strong> Define a point.</p>\n<pre><code>r.table('geo').insert({\n    id: 1,\n    name: 'San Francisco',\n    location: r.point(-122.423246,37.779388)\n}).run(conn, callback);\n</code></pre>",
    io: [['r', 'point']],
    name: 'point',
    url: 'point',
  },
  'api/javascript/polygon/': {
    ...
    example:
      "<p><strong>Example:</strong> Define a polygon.</p>\n<pre><code>r.table('geo').insert({\n    id: 101,\n    rectangle: r.polygon(\n        [-122.423246,37.779388],\n        [-122.423246,37.329898],\n        [-121.886420,37.329898],\n        [-121.886420,37.779388]\n    )\n}).run(conn, callback);\n</code></pre>",
    io: [['r', 'polygon']],
    name: 'polygon',
    url: 'polygon',
  },
  'api/javascript/polygon_sub/': {
    ...
    description:
      '<p>Use <code>polygon2</code> to "punch out" a hole in <code>polygon1</code>. <code>polygon2</code> must be completely contained within <code>polygon1</code> and must have no holes itself (it must not be the output of <code>polygonSub</code> itself).</p>',
    example:
      '<p><strong>Example:</strong> Define a polygon with a hole punched in it.</p>\n<pre><code>var outerPolygon = r.polygon(\n    [-122.4,37.7],\n    [-122.4,37.3],\n    [-121.8,37.3],\n    [-121.8,37.7]\n);\nvar innerPolygon = r.polygon(\n    [-122.3,37.4],\n    [-122.3,37.6],\n    [-122.0,37.6],\n    [-122.0,37.4]\n);\nouterPolygon.polygonSub(innerPolygon).run(conn, callback);\n</code></pre>',
    io: [['polygon', 'polygon']],
    name: 'polygonSub',
    url: 'polygon_sub',
  },
  ...
  'api/javascript/random/': {
    ...
    example:
      '<p><strong>Example:</strong> Generate a random number in the range <code>[0,1)</code></p>\n<pre><code>r.random().run(conn, callback)\n</code></pre>',
    io: ['r', 'number'],
    name: 'random',
    url: 'random',
  },
  'api/javascript/range/': {
    ...
    example:
      '<p><strong>Example:</strong> Return a four-element range of <code>[0, 1, 2, 3]</code>.</p>\n<pre><code>&gt; r.range(4).run(conn, callback)\n// result returned to callback\n[0, 1, 2, 3]\n</code></pre>',
    io: [['r', 'stream']],
    name: 'range',
    url: 'range',
  },
  'api/javascript/rebalance/': {
    ...
    description:
      '<p>Rebalances the shards of a table. When called on a database, all the tables in that database will be rebalanced.</p>',
    example:
      "<p><strong>Example:</strong> Rebalance a table.</p>\n<pre><code>&gt; r.table('superheroes').rebalance().run(conn, callback);\n</code></pre>",
    io: [
      ['table', 'object'],
      ['database', 'object'],
    ],
  ...
  'api/javascript/seconds/': {
    ...
    example:
      '<p><strong>Example:</strong> Return the post submitted during the first 30 seconds of every minute.</p>\n<pre><code>r.table("posts").filter(function(post) {\n    return post("date").seconds().lt(30)\n})\n</code></pre>',
    io: [['time', 'number']],
    name: 'seconds',
    url: 'seconds',
  },
  ...
  'api/javascript/set_intersection/': {
    ...
    description:
      '<p>Intersect two arrays returning values that occur in both of them as a set (an array with\ndistinct values).</p>',
    example:
      "<p><strong>Example:</strong> Check which pieces of equipment Iron Man has from a fixed list.</p>\n<pre><code>r.table('marvel').get('IronMan')('equipment').setIntersection(['newBoots', 'arc_reactor']).run(conn, callback)\n</code></pre>",
    io: [['array', 'array']],
  ...
  'api/javascript/set_union/': {
    ...
    example:
      "<p><strong>Example:</strong> Retrieve Iron Man's equipment list with the addition of some new boots and an arc reactor.</p>\n<pre><code>r.table('marvel').get('IronMan')('equipment').setUnion(['newBoots', 'arc_reactor']).run(conn, callback)\n</code></pre>",
    io: [['array', 'array']],
    name: 'setUnion',
    url: 'set_union',
  },
  'api/javascript/skip/': {
    ...
    example:
      "<p><strong>Example:</strong> Here in conjunction with <a href=\"/api/javascript/order_by/\">orderBy</a> we choose to ignore the most successful heroes.</p>\n<pre><code>r.table('marvel').orderBy('successMetric').skip(10).run(conn, callback)\n</code></pre>",
    io: [
      ['sequence', 'stream'],
      ['array', 'array'],
    ],
  ...
  'api/javascript/table_drop/': {
    ...
    description:
      '<p>Drop a table from a database. The table and all its data will be deleted.</p>',
    example:
  ...
  'api/javascript/time_of_day/': {
    ...
    description:
      '<p>Return the number of seconds elapsed since the beginning of the day stored in the time object.</p>',
    example:
      '<p><strong>Example:</strong> Retrieve posts that were submitted before noon.</p>\n<pre><code>r.table("posts").filter(\n    r.row("date").timeOfDay().le(12*60*60)\n).run(conn, callback)\n</code></pre>',
    io: [['time', 'number']],
    name: 'timeOfDay',
    url: 'time_of_day',
  },
  'api/javascript/timezone/': {
    ...
    example:
      '<p><strong>Example:</strong> Return all the users in the "-07:00" timezone.</p>\n<pre><code>r.table("users").filter( function(user) {\n    return user("subscriptionDate").timezone().eq("-07:00")\n})\n</code></pre>',
    io: [['time', 'time']],
    name: 'timezone',
    url: 'timezone',
  },
  ...
  'api/javascript/to_epoch_time/': {
    ...
    example:
      '<p><strong>Example:</strong> Return the current time in seconds since the Unix Epoch with millisecond-precision.</p>\n<pre><code>r.now().toEpochTime()\n</code></pre>',
    io: [['time', 'number']],
    name: 'toEpochTime',
    url: 'to_epoch_time',
  },
  'api/javascript/to_geojson/': {
    ...
    example:
      "<p><strong>Example:</strong> Convert a ReQL geometry object to a GeoJSON object.</p>\n<pre><code>r.table('geo').get('sfo')('location').toGeojson.run(conn, callback);\n// result passed to callback\n{\n    'type': 'Point',\n    'coordinates': [ -122.423246, 37.779388 ]\n}\n</code></pre>",
    io: [['geometry', 'object']],
    name: 'toGeojson',
    url: 'to_geojson',
  },
  'api/javascript/to_iso8601/': {
    ...
    example:
      '<p><strong>Example:</strong> Return the current ISO 8601 time.</p>\n<pre><code>r.now().toISO8601().run(conn, callback)\n// Result passed to callback\n"2015-04-20T18:37:52.690+00:00"\n</code></pre>',
    io: [['time', 'string']],
    name: 'toISO8601',
    url: 'to_iso8601',
  },
  'api/javascript/to_json_string/': {
    ...
    description:
      '<p>Convert a ReQL value or object to a JSON string. You may use either <code>toJsonString</code> or <code>toJSON</code>.</p>',
    example:
      '<p><strong>Example:</strong> Get a ReQL document as a JSON string.</p>\n<pre><code>&gt; r.table(\'hero\').get(1).toJSON()\n// result returned to callback\n\'{"id": 1, "name": "Batman", "city": "Gotham", "powers": ["martial arts", "cinematic entrances"]}\'\n</code></pre>',
    io: [['value', 'string']],
  ...
  'api/javascript/type_of/': {
    ...
    example:
      '<p><strong>Example:</strong> Get the type of a string.</p>\n<pre><code>r.expr("foo").typeOf().run(conn, callback);\n// Result passed to callback\n"STRING"\n</code></pre>',
    io: [['any', 'string']],
    name: 'typeOf',
    url: 'type_of',
  },
  ...
  'api/javascript/uuid/': {
    ...
    description:
      "<p>Return a UUID (universally unique identifier), a string that can be used as a unique ID. If a string is passed to <code>uuid</code> as an argument, the UUID will be deterministic, derived from the string's SHA-1 hash.</p>",
    example:
      '<p><strong>Example:</strong> Generate a UUID.</p>\n<pre><code>&gt; r.uuid().run(conn, callback)\n// result returned to callback\n"27961a0e-f4e8-4eb3-bf95-c5203e1d87b9"\n</code></pre>',
    io: [['r', 'string']],
    name: 'uuid',
    url: 'uuid',
  },
  ...
  'api/javascript/wait/': {
    ...
    example:
      '<p><strong>Example:</strong> Wait on a table to be ready.</p>\n<pre><code>&gt; r.table(\'superheroes\').wait().run(conn, callback);\n// Result passed to callback\n{ "ready": 1 }\n</code></pre>',
    io: [
      ['table', 'object'],
      ['database', 'object'],
      ['r', 'object'],
  ...
  'api/javascript/year/': {
    ...
    example:
      '<p><strong>Example:</strong> Retrieve all the users born in 1986.</p>\n<pre><code>r.table("users").filter(function(user) {\n    return user("birthdate").year().eq(1986)\n}).run(conn, callback)\n</code></pre>',
    io: [['time', 'number']],
    name: 'year',
    url: 'year',
  },
